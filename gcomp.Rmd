...
This is a marginal causal effect! Note that the estimate in the outcome_model produces a conditional effect (conditional on C1-C3).
...
Make function to perform g-computation on a single bootstrap sample. The `boot::boot` function requires the statistic functino to have two parameters: `data`, the original dataset, and `indices`, the indices for the bootstrap sample.
...
Now let's run to get a bootstrap estimate and confidence interval.
```{r}
set.seed(123)
n_reps <- 100
